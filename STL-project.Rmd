...
# Then we fit the two models to the point and arc target data for each of the participants. This step in particular might take a LONG time, as it fits 2 models each to 2 subsets of the data of 129 participants.
bothModelFits()

# To be able to see how well STL maps onto initial responses in a longer exposure phase, people did a 20 degree rotation for 160 trials. The reach deviation on the second trial should be equal to what the functions fitted to the STL data predict at 20 degree rotations. To get reduced noise estimates of what happens on the second trial, we fit an exponential to each participants learning curve:
getLongExpFits()
...
We can see that the effect is there in both groups, but is only clearly significant in the 60 degree group. This could be because of a stimulus range effect (up to 45 degrees or up to 60 degrees) or because there are 31 participants in the 45 degree group, while the 40 degree group has a little more power with 43 participants.
...
We could also plot the average errors of the model predictions over the rotation size.
...
With adjusted R-squared of 0.39 or higher, and all p-values well below 0.001, all fits are reasonable. The AIC's for the linear fits are also very similar for the capped and attribution models, in all 3 groups, again indicating no difference between the two approaches.
